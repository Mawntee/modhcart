...
//======== INPUT/OUTPUT ======== 
  ...
  const DifSelection  = 1                       // int - Set difficulty files based on 'if' statements below
  const AddCustomData = true;                   // t/f - Adds customData arrays to objects  --> Set 'true' if modhcart -> 'false' for vanilla
  const precision     = 6;                      // int - Decimals to round to               --> Use this for cleaner anims or to try and decrease output file size
  const lightshow     = false;                  // t/f - Import lightshow from other file   --> Set 'lightsPath' below
  const clearEnv      = true;                   // t/f - Overwrite custom environment data from input dif
  const lightsPath    = "EasyLightshow.dat";    // "string" - Currently only supports Chroma/Vanilla lightshows made in Chrommaper using the V3 format.             
  const lightsEnv     = true;                   // t/f - Import Chroma Environment Enhancements, Geometry, and Materials from lightshow dif
  const lightsTracks  = true;                   // t/f - Import Noodle/Chroma custom events from lightshow dif
  
  const requirements  = ["Chroma", "Noodle Extensions"];  // ["string", "array"] Chromapper likes to mess with these. Use this to tell it who's boss >:3 | (set to 'false' if no requirements are required)

//======== CringeFix™ =========                 // Various Tools and fixes for mod/config conflicts

  const CringeFix       = true;                 // t/f - Fixes for common mod conflicts such as JDFixer and BS+ note scaling --> Enables lower options
  const ForceNJS        = true;                 // t/f/num - Force NJS into every playable object --> 'true' samples info.dat, or you can enter in any value
  const ForceJD         = 0;                    // t/f/num - Bakes spawn offset into every playable object (JD/RT) -> (same as above)    
  const ForceNoteScale  = true;                 // t/f/[x,y,z] - Prevents BS+, Adaptable Notes, and similar note tweaks mods from causing mismatched note scaling between vanilla/animated notes
  const ForceBombScale  = true;                 // t/f/[x,y,z] - Same as above -> Both can be 'true', 'false', or an array -> Example: [0.5, 0.5, 0.5] for half sized notes/bombs         
  const FixAllArtArcs   = true;                 // t/f - Cleans up notes attached to any arcs marked as "uninteractable: true" -> Removes any real or fake notes attached to head/tail of arcs, then creates an invisible dummy head note
...
//==========[ DEBUG ]======================================================================================================================================
  // - Reaction Time -
  const showRTGuides    = true;           // Spawn guidelines in world to help visualize reaction time when animating notes/walls
  ...
  const adaptiveRTAdj   = false;          // Scales transition time based on lifetime of incoming notes/walls/etc
  const hideRTStats     = true;          // Prevents logging of NJS stats and updates.
  const hideRTYapping   = false;          // Removes very important information that you should definitely read and understand first! :)
  

  const logStyling      = [4, 621]; //[0, 621];       // false = boring | 0 = basic | 1-9 = LGBTQ-GF-NB-PS-A | 69 = rainbow | [int, 621] = furry
/*=========================================================================================================================================================
...
function _MawnteeTouchedMe() {
  ...
  infoDat._customData._editors.MawnteesStinkyScript.version = `420.69.${scriptVer}.621`
...
function preventCRINGE_OffsNJS(NJS, offset) {
  ...
  if (NJS === undefined || offset === undefined) {
    ...
    if (offsNJS) {
        console.log(`${footer} Pulling NJS/Offset from info.dat${reset}`);
        if (NJS === undefined) {
          NJS = offsNJS.NJS;
        }
        if (offset === undefined) {
        ...
        if (NJS === undefined && offset === undefined) {
          if (ForceJD === true && ForceNJS === true) {
            NJS = 18;
            offset = 0;
            console.log(' WARNING: Difficulty data not found and no NJS/offset values specified. Defaulting NJS to 18 and offset to 0');
            console.log("");
          }
        } else if (NJS === undefined) {
          if (ForceNJS === true) {
            NJS = 18;
            console.log(' WARNING: Difficulty data not found and no NJS value specified. Defaulting NJS to 18');
            console.log("");
          }
        } else if (offset === undefined) {
          if (ForceJD === true) {
            offset = 0;
            console.log(' WARNING: Difficulty data not found and no offset value specified. Defaulting offset to 0');
            console.log("");
          }
        }
    }
  ...
  if (ForceNJS === false) {
    console.log(`${bodyText} - NJS not forced${reset}`)
  } else {
    notes.forEach(note => {
    ...
    console.log(`${bodyText} - Jump speed of all unset objects set to:`  + ` ${valueText}`+ NJS + ` njs${reset}`);
  }
  
  if (ForceJD === false) {
    console.log(`${bodyText} - JD not forced`)
  } else {
    notes.forEach(note => {
  ...
  //console.log(` ${green}NJS/Offset Stats: (placeholder)${reset}`);
  //console.log(`  Min/Max NJS  =` + ` ${valueText}` + NJS       + `${reset}`);
  //console.log(`  Forced JD    =` + ` ${valueText}` + ForceJD   + `${reset}`);
  //console.log(`  Forced NJS   =` + ` ${valueText}` + ForceNJS  + `${reset}`);
  console.log("");
  return (offset, NJS);
}
...
function preventCRINGE_Scaling(ForceNoteScale, ForceBombScale) {
  ...
  if (ForceNoteScale || ForceBombScale) {
    console.log(`${header}${bold}Forced Scaling Values:${reset}`);
  }
  // assign scaling
 ...
 *
 * - unique track names for each lane
 * 
 * - will not overwrite old track names
 * 
 * - {@link track} can be a single "string" or an ["array", "of", "strings"]
...
// buncha defines to be used later by the shit below
...
function createStaticRT (desiredRT, spawnBeat = undefined, endBeat = undefined, njs = undefined, offset = undefined, startCol = undefined, endCol = undefined) {
  console.log(`${header}${bold}Custom RT Indicator Added:${reset}`);
  ...
  if (!desiredRT) {
    if (baseRT > minRT){
      desiredRT = (minRT * (1 - 0.5)) + (baseRT * 0.5);
      const undefSafeRTLog = `${red}  - Desired RT undefined${bodyText} -> Setting Comfy RT indicator to: ${valueText}${desiredRT}ms${reset}`
    ...
    if (baseRT <= minRT){
      ...
      desiredRT = minRT+50;
      const zoomerRTFixLog = `${bodyText} Setting desired reaction time guideline to: ${valueText}${desiredRT}ms${bodyText} (min + 50ms)${reset}`
      console.log(zoomerRTFixLog);
    }
  }
  // Calculate distances
  let desiredRTOffs   = getOffsetFromReactionTime(desiredRT,songData.NJS,BPM);
  _JDist          = getJumpZPos(songData.NJS, songData.offset, BPM);
  _HJDist         = (getJumpZPos(songData.NJS, songData.offset, BPM)/2);
  _DesiredDist    = getJumpZPos(songData.NJS, desiredRTOffs, BPM);
  _PANIKDist      = (getJumpZPos(songData.NJS, desiredRTOffs, BPM)/2);
  _baseJDist      = _JDist;
  _baseHJDist     = _HJDist;
  ...
  // Log RT stats
  if (!hideRTStats) {
    console.log(`${bodyText}  Reaction Time Guides Created:${reset}`);
  ...
  materials.DefaultJumpDist = ({
    color: [1, 1, 1, 0],
    shader: "Standard",
    "shaderKeywords": []
  }); 
  materials.DefaultHalfJumpDist = ({
    color: [0, 0, 1, 0],
    shader: "Standard",
    "shaderKeywords": []
  });
  materials.IntendedReactionTime = ({
    color: [0, 1, 0, 0],
    shader: "Standard",
    "shaderKeywords": []
  });
  materials.MinimumReactionTime = ({
    color: [1, 0, 0, 0],
    shader: "Standard",
    "shaderKeywords": []
  });
  ...
  environment.push({
    geometry: {
      type: "Cube",
      material: "DefaultJumpDist",
    },
    scale: [1.15, 0.115, 0.069],
    track: "mwt_Debug_Wall_RT_NoteJump",
    position: [(20*0.06)+1.15, 0.115, _JDist]
  },{
    geometry: {
      type: "Cube",
      material: "DefaultHalfJumpDist",
    },
    scale: [1.15, 0.115, 0.069],
    track: "mwt_Debug_Wall_RT_NoteHalfJump",
    position: [(20*0.06)+1.15, 0.115, _HJDist]
  });
  
  // Cringe nay nay walls
  obstacles.push({
    b: 0,  // Beat
    d: 9999, // Duration
    x: 0,   // Line Index
  ...
  _currentCustJDist   = _JDist;
  _currentCustHJDist  = _HJDist;
  _currentDesiredDist = _DesiredDist;
  _currentPANIKDist   = _PANIKDist;
  ...
  customEvents.push({
    ...
    d: {
      track: "mwt_Debug_Wall_RT_NoteJump",
      position: [[0, guideHeight*2, _currentCustJDist, 0]],
    }
  },{
    b: 0,
    t: "AnimateTrack",
    d: {
      track: "mwt_Debug_Wall_RT_NoteHalfJump",
      position: [[0, guideHeight*2, _currentCustHJDist, 0]],
    }
  },{
    b: 0,
    ...
    d: {
      track: "mwt_Debug_Wall_Secondary_RT_NoteJump",
      definitePosition: [[0, guideHeight*2, _currentCustJDist/0.6, 0]],
    }
  },{
    b: 0,
    t: "AssignPathAnimation",
    d: {
      track: "mwt_Debug_Wall_Secondary_RT_NoteHalfJump",
      definitePosition: [[0, guideHeight*2, _currentCustHJDist/0.6, 0]], 
    }
  },{
    b: 0,
    t: "AssignPathAnimation",
    d: {
      track: "mwt_Debug_Wall_RT_SAFE",
      definitePosition: [[0, guideHeight, _currentDesiredDist/0.6, 0]],
    }
  },{
    b: 0,
    t: "AssignPathAnimation",
    d: {
      track: "debugGeo_RT_PANIK",
      definitePosition: [[0, guideHeight, _currentPANIKDist/0.6, 0]],
    }
  },{
    b: 0,
    t: "AnimateTrack",
    d: {
      track: "debugGeo_RT_SAFE",
      position: [[0, guideHeight, _currentDesiredDist, 0]],
    }
  },{
    b: 0,
    t: "AnimateTrack",
    d: {
      track: "debugGeo_RT_PANIK",
      position: [[0, guideHeight, _currentPANIKDist, 0]],
    }
  });
}
//createStaticRT(desiredRT);
...
function adjustDynRTGuides(beat, newNJS, newOffset, newRT, animDuration = undefined) {
  ...
  if (!hideRTStats) {
    ...
    //console.log(`${black}   Function Started on Beat: [${white} ${beat} ${black}]${reset}`);
...
offsetNJSOnNotesBetween(22, 38, 14,  2);
offsetNJSOnNotesBetween(38, 54, 19, -0.25);
offsetNJSOnNotesBetween(54, 69, 12,  0);
offsetNJSOnNotesBetween(70, 86, 18, -0.5);
offsetNJSOnNotesBetween(86, 94, 18,  4);
resetRTGuides(94); 
...
===============================================================
  ...
  filteredNotes.forEach(note => {
    ...
    note.customData. // insert property type here from example below
  });
  ...
  filteredNotes.forEach(note => {
    note.customData.noteJumpMovementSpeed = 420;
    note.customData.noteJumpStartBeatOffset = 69;
    note.customData. // insert property type here from examples below
  });
...
environment.push({
  id: "^GameCore\\.\\[\\d*\\]PanelLightTrackLaneRing\\(Clone\\)\\.\\[\\d*\\]Ring$",
  lookupMethod: "Regex",
  active: false,
}
,{
...
if (lightshow) {
  ...
  var LightsIn = JSON.parse(fs.readFileSync(lightsPath));
  
  for(var diffPath of diffPaths) {
    var j = JSON.parse(fs.readFileSync(diffPath));
  
    j.basicBeatmapEvents = [...(j.basicBeatmapEvents     || []), ...(LightsIn.basicBeatmapEvents      || [])].sort((a, b) => a.b - b.b);
    
if (lightsEnv) {
  j.customData.environment = [
    ...(j.customData.environment || []),  // Existing environment objects
    ...(LightsIn.customData.environment || []) // New environment objects
  ];
  j.customData.materials = {
    ...(j.customData.materials || {}),  // Existing materials object
    ...(LightsIn.customData.materials || {}) // New materials from lightshow
  };
}
